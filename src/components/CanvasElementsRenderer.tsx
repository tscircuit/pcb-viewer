...
export const CanvasElementsRenderer = (props: CanvasElementsRendererProps) => {
  ...
  const [xRayNetId, setXRayNetId] = useState<string | null>(null)
  // Net IDs are generated by the connectivity map and may change with the scene.
  useEffect(() => setXRayNetId(null), [connectivityMap])
      ...
      xRayNetId
        ? elementsToRender.filter(
            (element) =>
              ...
              getElementNetId(element, connectivityMap) === xRayNetId,
          )
        : undefined,
    [elementsToRender, connectivityMap, xRayNetId],
  )
  const getNetAtPoint = (point: { x: number; y: number }) => {
    ...
    // During X-Ray, the visible selected net takes precedence over copper below it.
    if (
      xRayNetId &&
      copper.some(
        (p) => getElementNetId(p._element, connectivityMap) === xRayNetId,
      )
    )
      return xRayNetId
    copper.sort(
