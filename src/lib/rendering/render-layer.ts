...
import type { RenderScene, LayerRenderRequest } from "./types"

/** Shared rasterizer: runs in a worker, or on a detached canvas in the fallback. */
export function renderLayer(
  scene: RenderScene,
  request: LayerRenderRequest,
  target: HTMLCanvasElement | OffscreenCanvas,
) {
  const { elements, primitives, options } = scene
  const {
    selectedLayer,
    hiddenLayerOpacity,
  ...
  // Both drawers only use the Canvas 2D surface API. Their upstream types still
  // name HTMLCanvasElement; keep the OffscreenCanvas compatibility cast here.
  const availableCanvasRefs = { [request.layer]: target as HTMLCanvasElement }
  const knownLayers = new Set(getOrderedCanvasLayers(elements))
